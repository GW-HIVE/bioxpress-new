...
def main():
    ...
    try:
        ...
        sql = "delete from biox_level"
        cur.execute(sql)
        
        #Reset auto increment
        sql = "ALTER TABLE biox_level AUTO_INCREMENT = 1"
        cur.execute(sql)
        dbh.commit()
	    ...
	    feature_id = genename2featureid[gene_name]
            ...
            sql = "INSERT INTO biox_level (featureId,sampleId,sigFlag,deDirection,dataSource,pmid,cancerId,subjectsUp,subjectsDown,subjectsNoChange,subjectsTotal,pValue,adjPvalue,logFc) VALUES " 
            sql += "(%s,%s,'%s','%s','%s','%s',%s,%s,%s,%s,%s,%s,%s,%s)" % (feature_id,sample_id,significance,direction,data_source,pmid,cancer_id,subjects_up,subjects_down,subjects_nochange,subjects_total,pvalue,adjpvalue,log2fc)
